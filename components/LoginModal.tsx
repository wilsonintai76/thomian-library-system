                    ...
                    <div className="p-6 space-y-4">
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="space-y-3">
                                <div className="bg-slate-50 border-2 border-blue-200 rounded-2xl px-4 py-3 text-left focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Staff ID</label>
                                    <input
                                        type="text"
                                        autoFocus
                                        value={username}
                                        onChange={(e) => setUsername(e.target.value)}
                                        className="w-full bg-transparent font-mono font-black text-2xl text-slate-800 tracking-widest outline-none placeholder:text-slate-300"
                                        placeholder="Enter ID"
                                    />
                                </div>
                                <div className="bg-slate-50 border-2 border-slate-200 rounded-2xl px-4 py-3 text-left focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                                    <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">Staff PIN</label>
                                    <input
                                        type="password"
                                        value={pin}
                                        onChange={(e) => setPin(e.target.value)}
                                        className="w-full bg-transparent font-mono font-black text-2xl text-slate-800 tracking-[0.5em] outline-none placeholder:text-slate-300"
                                        placeholder="••••"
                                        maxLength={PIN_MAX}
                                    />
                                </div>
                            ...
                            {/* Action buttons */}
                            <button type="submit" disabled={isLoading || pin.length < 4 || username.length < 4}
                                className="w-full bg-blue-600 text-white py-3.5 rounded-xl font-bold text-base hover:bg-blue-700 active:scale-[0.98] transition-all shadow-xl shadow-blue-100 disabled:opacity-50 flex items-center justify-center gap-2 mt-4">
                                {isLoading ? <div className="h-5 w-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 'Login'}
                            </button>
                        </form>
