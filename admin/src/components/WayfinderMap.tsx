...
const WayfinderMap: React.FC<WayfinderMapProps> = ({
    ...
    const getTargetShelf = (): MapElement | null => {
        ...
        // 1. Collect all elements of type SHELF across all levels
    ...
    if (!currentLevel) return <div className="p-20 text-center animate-pulse text-slate-400 font-black uppercase tracking-widest">Floor Plan Not Configured</div>;
            ...
            >
                ...
                <g>
                    {(currentLevel.wallSegments || []).map(seg => {
                        ...
                        // Build wall parts with gaps for features
                ...
                {/* LAYER 2b: NON-WALL ELEMENTS (Furniture, Shelves, Text) — from layout */}
                            ...
                            {el.type === 'SHELF' && (
                                <g>
                                    <rect 
                                        ...
                                        fill={isTarget ? 'rgba(59, 130, 246, 0.7)' : (heatmapData?.[el.id] ? getHeatColor(heatmapData[el.id]) : '#fff')} 
